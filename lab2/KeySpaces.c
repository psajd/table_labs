...
int search_ks1(Table *table, int k) {
    for (int i = table->ks1_offset; i < table->csize1 * sizeof(KeySpace1); i += sizeof(struct KeySpace1)) {
...
int garbage_collector_ks1(struct Table *table) {
    ...
    table->csize1 = 0;
    for (int i = table->ks1_offset; i < cur * sizeof(KeySpace1); i += sizeof(KeySpace1)) {
        fseek(table->file, i, SEEK_SET);
        KeySpace1 *temp = malloc(sizeof(KeySpace1));
        fread(temp, sizeof(KeySpace1), 1, table->file);
        if (temp->busy) /* элемент занят */
        {
            if (i != table->csize1 * sizeof(KeySpace1)) /* элемент должен быть перемещен */
            {
                ...
                fseek(table->file, table->csize1 * (int) sizeof(KeySpace1), SEEK_SET);
...
int insert_ks2(struct Table *table, int k, struct Item *item) {
    ...
    start = i;
    fseek(table->file, i * (int) sizeof(KeySpace2) + table->msize1 * (int) sizeof(KeySpace1), SEEK_SET);
...
int search_ks2(struct Table *table, int k) {
    ...
    while (temp_ks2->busy > 0) /* позиция не свободна */
    {
        ...
        {
            free(temp_ks2);
            return i * (int) sizeof(KeySpace2) + table->msize1 * (int) sizeof(KeySpace1);
        }
        i = (i + h) % table->msize2; /* следующая позиция */
        fseek(table->file, i * (int) sizeof(KeySpace2) + table->msize1 * (int) sizeof(KeySpace1), SEEK_SET);
        fread(temp_ks2, sizeof(KeySpace2), 1, table->file);
